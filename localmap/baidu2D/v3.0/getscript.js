...
(function() {
    ...
    ;function pa(a, b) {
        if (/^http/.test(a))  return;//cqy  屏蔽ak验证，若调用外部资源直接返回
        if (b) {
     ...
     //cqy 这里是所有的模块，用到时自动加载（在线），离线的话要先下载下来放到本地，如http://api0.map.bdimg.com/getmodules?v=3.0&mod=模块1,模块2 
    ;var Tb = {
        map: "i5s0dw",
        common: "jrzmva",
        style: "mqdswt",
            ...
            if (e.Ke == this.tj.Bp)
                ...
                if (e.Ke == this.tj.BF) {
                    ...
                    setTimeout(function() {
                        ...
                        //cqy 模块调用本地js（在 modules 目录下）
                        console.log(a);
                        0 == a.length ? f.XJ() : pa("http://api0.map.bdimg.com/getmodules?v=3.0&mod=" + a.join(","))
                        if( a.length > 0 ){
                            for(i=0; i<a.length;i++){
                              var md = MAPURL.host+'modules/' + a[i] + '.js';
                              pa(md);
                              console.log('加载模块:'+md);
                            }
                          } else {
